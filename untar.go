...
func UnTarIndex(ctx context.Context, fs FilesystemWriter, index Index, s Store, key []byte, n int, pb ProgressBar) error {
	...
	for i := 0; i < n; i++ {
		g.Go(func() error {
			for r := range req {
				...
				b, err := chunk.GetPackagedData(key, true, false)
				if err != nil {
					close(r.data)
					return err
				}
